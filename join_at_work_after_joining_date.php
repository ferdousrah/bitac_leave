...
<div class="row mb-4">
    <div class="col-12 col-md-6">
        <h4 class="fw-bold">বর্ধিত ছুটি মঞ্জুর ও কর্মস্থলে যোগদানের অনুমতি</h4>
    </div>
...
<div class="card">
    <div class="card-body">
        <form name="form" id="form">
            ...
            <div class="row mb-3 align-items-center">
                <label class="col-md-3 col-form-label fw-semibold">চাহিত ছুটির ধরণ</label>
                <div class="col-md-9">
                    <select class="form-select" name="approvedLeaveType" required>
                        <option value="">-- ছুটির ধরণ নির্বাচন করুন --</option>
                        <?php while ($lRow = mysqli_fetch_assoc($getAllLeaveTypesQ)): ?>
                        <option value="<?= $lRow['leaveID'] ?>"
                            <?= ($getLeaveApplicationDetailsQRW['approvedLeaveType'] == $lRow['leaveID']) ? 'selected' : '' ?>>
                            <?= htmlspecialchars($lRow['leaveTitle']) ?>
                        </option>
                        <?php endwhile; ?>
                    </select>
                </div>
            ...
            <div class="row mb-3 align-items-center">
                <label class="col-md-3 col-form-label fw-semibold">লিভ টেম্পলেট</label>
                <div class="col-md-9">
                    <select class="form-select" onchange="insertTemplate(this.value)">
                        <option value="">-- টেম্পলেট নির্বাচন করুন --</option>
                        <?php while ($ltRow = mysqli_fetch_assoc($getLeaveTemplatesQ)): ?>
                        <option value="<?= htmlspecialchars($ltRow['templateData']) ?>">
                            <?= htmlspecialchars($ltRow['templateData']) ?>
                        </option>
                        <?php endwhile; ?>
                    </select>
                </div>
            ...
            <div class="row mb-3 align-items-center">
                <label class="col-md-3 col-form-label fw-semibold">বিলম্বের কারণ</label>
                <div class="col-md-9">
...
$(document).ready(function () {
    ...
    form.on('submit', function (e) {
        ...
        $.ajax({
            url: 'insert_after_leave_joining_application_action.php',
            type: 'POST',
            dataType: 'html',
