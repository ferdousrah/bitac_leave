...
// Use pre-set assetsPath if available (set by inline script), otherwise read from data attribute
window.assetsPath = window.assetsPath || document.documentElement.getAttribute('data-assets-path') || 'vuexy-assets/';
window.templateName = window.templateName || document.documentElement.getAttribute('data-template') || 'vertical-menu-template';
...
// Only create TemplateCustomizer if it doesn't already exist (prevents reinitialization issues)
