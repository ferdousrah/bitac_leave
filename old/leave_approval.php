...
$(document).ready(function() {
    // Initialize Supervision (সুপারিশ) DataTable
    leaveTableInstance = $('#leaveTable').DataTable({
        processing: true,
        serverSide: true,
        ajax: {
            url: "fetch_waitingtosupervise_leave_data.php",
            type: "POST",
    ...
    // Initialize Approval (অনুমোদন) DataTable (lazy load on tab switch)
    $('button[data-bs-target="#approval"]').on('shown.bs.tab', function (e) {
        if (!leaveApproveTableInstance) {
            leaveApproveTableInstance = $('#leaveApproveTable').DataTable({
                processing: true,
                serverSide: true,
                ajax: {
                    url: "fetch_waitingtoapprove_leave_data.php",
                    type: "POST",
                    dataSrc: function (json) {
                        console.log(json);
                        return json.data;
