...
function generatePDFData($leaveApplicationID) {
    try {
        ...
        $orgId = (int)$empData['organization_id'];
        ...
        if (!empty($leaveData['signature'])) {
            ...
            if (!empty($sigBase64)) {
                $html .= '<img src="data:image/png;base64,' . $sigBase64 . '" style="max-width:150px; max-height:80px; display:block; margin:5px 0;" /><br>';
            }
        }
        ...
        if ($supervisorApproval && !empty($supervisorApproval['signature'])) {
            $sig = $supervisorApproval['signature'];
            $decoded = @base64_decode($sig, true);
            $sigBase64 = ($decoded !== false && @imagecreatefromstring($decoded)) ? $sig : base64_encode($sig);
            if (!empty($sigBase64)) {
                $html .= '<img src="data:image/png;base64,' . $sigBase64 . '" style="max-width:150px; max-height:80px; display:block; margin:5px 0;" /><br>';
...
<body>
    <div class="header">
        <h1>📄 Leave Application - ID: <?= $leaveApplicationID ?></h1>
    </div>
