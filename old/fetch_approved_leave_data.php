...
while ($empRow = mysqli_fetch_array($query)) {
    ...
    if($proceed == 1){
        ...
        if($getLeaveApplicationDetailsQRW['leaveTypeInTwo'] == 1){
            $proposed_leave_type = "গড় বেতন ";
        }else if($getLeaveApplicationDetailsQRW['leaveTypeInTwo'] == 2){
            $proposed_leave_type = "অর্ধ-গড় বেতন ";
            ...
            $proposed_leave_type = "অধ্যয়ন ছুটি";
        ...
        // Action buttons with Bootstrap 5 and Tabler icons
        $action = '<div class="btn-group" role="group">';
        $action .= '<a class="btn btn-sm btn-info" target="_blank" href="leave_application_details.php?menuslug=leave-approval&leaveApplicationID=' . $empRow['leaveApplicationID'] . '">';
        $action .= '<i class="ti tabler-file-text me-1"></i>আবেদনপত্র';
        $action .= '</a>';

        if ($getLeaveApplicationDetailsQRW['attachment'] != '') {
            $action .= '<a class="btn btn-sm btn-secondary" target="_blank" href="uploads/' . htmlspecialchars($getLeaveApplicationDetailsQRW['attachment']) . '">';
            $action .= '<i class="ti tabler-paperclip me-1"></i>সংযুক্তি';
            $action .= '</a>';
        }
        ...
        $nestedData['applicant_name'] = htmlspecialchars($empRow['applicant_name'].', '.$getDesignationDetailsQRW['job_title_name']);
        $nestedData['section'] = htmlspecialchars($getSectionDetailsQRW['section_name'].", ".$getorgDetailsQRW['organization_name']);
        $nestedData['leave_type'] = htmlspecialchars($getLeaveTypeQRW['leaveTitle']);
        ...
        $nestedData['proposed_leave_type'] = htmlspecialchars($proposed_leave_type);
