...
if ($hasDeletedColumn) {
    $checkQuery = "SELECT id FROM user_group WHERE group_name = ? AND id != ? AND deleted = 0";
} else {
    $checkQuery = "SELECT id FROM user_group WHERE group_name = ? AND id != ?";
}
...
mysqli_stmt_bind_param($checkStmt, "si", $group_name, $dataID);
...
// Update user group
if ($hasDeletedColumn) {
    $updateQuery = "UPDATE user_group SET group_name = ?, display_order = ? WHERE id = ? AND deleted = 0";
} else {
    $updateQuery = "UPDATE user_group SET group_name = ?, display_order = ? WHERE id = ?";
}
...
mysqli_stmt_bind_param($updateStmt, "sii", $group_name, $display_order, $dataID);
...
if (mysqli_stmt_execute($updateStmt)) {
    ...
    echo json_encode(['status' => 0, 'message' => 'ব্যবহারকারী গ্রুপ আপডেট করতে ব্যর্থ হয়েছে!']);
}
