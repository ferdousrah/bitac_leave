...
// Canonical "current signatory" filter for the post-forward chain:
// - My row must be pending (isApproved=0)
// - Not the supervisor row (supervisors act via supervisor-queue.php)
// - Chain must be unlocked by center admin (isSentbyAdmin=1)
// - AND either I'm first non-supervisor with prev approved, OR my prev is approved
$where = "opa.status = 0
...
$sql = "
    SELECT opa.*, s.dataID AS sigDataID, s.serial, s.isSupervisor,
...
while ($row = mysqli_fetch_assoc($res)) {
    ...
    $action = '<div class="action-group">'
            ...
            . '<button type="button" class="action-icon icon-approve btn-opa-approve" data-pid="' . $pid . '" data-bs-toggle="tooltip" title="অনুমোদন"><i class="ti tabler-check"></i></button>'
    ...
    $data[] = [
        ...
        'notes'       => $notesCell,
        'submit_date' => bn_dt_short($row['submit_date']),
        'action'      => $action,
    ];
    $sl++;
}
