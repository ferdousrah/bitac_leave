...
if (!empty($search)) {
    if ($whereClause === "") {
        $whereClause = " WHERE section_name LIKE ?";
    } else {
...
$totalQuery = "SELECT COUNT(*) as total FROM sections";
if ($hasDeletedColumn) {
...
$filteredQuery = "SELECT COUNT(*) as total FROM sections" . $whereClause;
if (!empty($params)) {
...
// Get actual data
$sql = "SELECT * FROM sections" . $whereClause . " ORDER BY {$orderBy} {$orderDirection} LIMIT ?, ?";
...
while ($row = mysqli_fetch_assoc($result)) {
    $menuslug = isset($_GET['menuslug']) ? htmlspecialchars($_GET['menuslug']) : 'manage-sections';

    $sectionName = htmlspecialchars($row['section_name']);
    $nameCell = '<div class="d-flex align-items-center gap-2"><span class="section-icon-tile"><i class="ti tabler-building"></i></span><span class="center-name">' . $sectionName . '</span></div>';
