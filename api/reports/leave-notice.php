<?php
/**
 * Leave Office Order Viewer - In-Memory Version
 * Shows official leave approval notice
 * 
 * Usage: leave_office_order_viewer.php?leaveApplicationID=5516
 */
...
if ($leaveApplicationID <= 0) {
    ...
    <body>
        <div class="error-box">
            ...
            <div class="info">
                <strong>Usage:</strong>
                <div class="example">leave_office_order_viewer.php?leaveApplicationID=<strong>5516</strong></div>
            </div>
        </div>
    </body>
    </html>
...
function generatePDFData($leaveApplicationID) {
    try {
        ...
        // Get approved leave type
        $stmt = $con->prepare("SELECT * FROM leave_types WHERE leaveID = ?");
        $stmt->bind_param("s", $leaveData['approvedLeaveType']);
        $stmt->execute();
        ...
        $officeNoticeDate = date_create($leaveData['officeNoticeDate']);

        // ── Fetch proposed segments (final approved breakdown) ──
        $hasKindCol = false;
        $colC = $con->query("SHOW COLUMNS FROM leave_application_segments LIKE 'kind'");
        if ($colC && $colC->num_rows > 0) $hasKindCol = true;
        $kindFilter = $hasKindCol ? " AND s.kind = 'proposed' " : "";
        ...
        $segS->execute();
        $noticeSegRows = $segS->get_result()->fetch_all(MYSQLI_ASSOC);
        $segS->close();
        $isNoticeMulti = count($noticeSegRows) > 1;
        $totalNoticeDays = $isNoticeMulti ? array_sum(array_column($noticeSegRows, 'days')) : $dateDiff;
        
        // Get signatory (serial 2)
        $stmt = $con->prepare("SELECT * FROM leave_data_for_approval WHERE leaveApplicationID = ? AND serial = '2'");
        $stmt->bind_param("i", $leaveApplicationID);
        $stmt->execute();
        $signatoryApproval = $stmt->get_result()->fetch_assoc();
        $stmt->close();
        ...
        if ($signatoryApproval && !empty($signatoryApproval['signatory'])) {
            // Get signatory employee details
            $stmt = $con->prepare("SELECT * FROM employee_list WHERE id = ?");
            $stmt->bind_param("i", $signatoryApproval['signatory']);
            $stmt->execute();
            ...
            if ($signatoryData) {
                ...
                $stmt = $con->prepare("SELECT * FROM user_list WHERE employee_id = ?");
                $stmt->bind_param("i", $signatoryData['id']);
                $stmt->execute();
                $sigUserData = $stmt->get_result()->fetch_assoc();
                $stmt->close();
                
                // Get designation
                $stmt = $con->prepare("SELECT * FROM job_title WHERE id = ?");
                $stmt->bind_param("i", $signatoryApproval['designation_id']);
                $stmt->execute();
                ...
                // Get section
                $stmt = $con->prepare("SELECT * FROM sections WHERE id = ?");
                $stmt->bind_param("i", $signatoryApproval['section_id']);
                $stmt->execute();
                ...
                // Get organization
                $stmt = $con->prepare("SELECT * FROM organization WHERE id = ?");
                $stmt->bind_param("i", $signatoryApproval['organization_id']);
                $stmt->execute();
        ...
        $html .= htmlspecialchars($orgData['address']) . '<br><br>';
        $html .= '</p>';
        
        // Notice number and date
        $html .= '<table>';
        $html .= '<tr>';
        $html .= '<td width="50%"><p>নং- ' . htmlspecialchars($empData['memorialNo']) . '.' . banglaNumber($leaveData['officeNoticeNumber']) . '</p></td>';
        $html .= '<td width="50%" style="text-align: right;"><p class="text-right">তারিখঃ ' . banglaNumber(date_format($officeNoticeDate, "d/m/Y")) . ' খ্রিস্টাব্দ</p></td>';
        $html .= '</tr>';
        $html .= '</table>';
        ...
        $html .= htmlspecialchars($orgData['organization_name']) . 'র ';
        $html .= banglaNumber(date_format($submitDate, "d/m/Y")) . ' ইং তারিখের আবেদনের প্রেক্ষিতে বিভাগীয় প্রধানের সুপারিশের আলোকে তার ';

        if ($isNoticeMulti) {
            // Multi-segment: enumerate each segment with its dates and type
            $segParts = [];
            foreach ($noticeSegRows as $sg) {
                $segParts[] = banglaNumber(date('d/m/Y', strtotime($sg['dateFrom']))) . ' ইং থেকে '
                            . banglaNumber(date('d/m/Y', strtotime($sg['dateTo']))) . ' ইং তারিখ পর্যন্ত '
                            . banglaNumber((int)$sg['days']) . ' দিনের '
                            . htmlspecialchars($sg['leaveTitle'] ?? 'অজানা');
            }
            if (count($segParts) === 2) {
                $html .= $segParts[0] . ' এবং ' . $segParts[1];
            } else {
                $last = array_pop($segParts);
                $html .= implode(', ', $segParts) . ' এবং ' . $last;
            }
            $html .= ' — সর্বমোট ' . banglaNumber($totalNoticeDays) . ' দিনের ছুটি মঞ্জুর করা হলো।';
        } else {
            $html .= banglaNumber(date_format($dateF, "d/m/Y")) . ' ইং থেকে ';
            $html .= banglaNumber(date_format($dateT, "d/m/Y")) . ' ইং তারিখ পর্যন্ত ';

            // Conditional text based on leave type
            if ($leaveData['leaveTypeInTwo'] == 2) {
                $dateDifftwo = $dateDiff * 2;
                $html .= 'মোট ' . banglaNumber($dateDiff) . ' দিনের (';
                $html .= banglaNumber($dateDifftwo) . ' দিনের অর্ধ-গড় বেতনের ছুটিকে ';
                $html .= banglaNumber($dateDiff) . ' দিনের গড় বেতনের ছুটিতে রূপান্তরিত করে) গড় বেতনে ছুটি মঞ্জুর করা হলো।';
            } else {
                $html .= banglaNumber($dateDiff) . ' দিনের ';
                $html .= htmlspecialchars($leaveTypeData['leaveTitle']) . ' ছুটি মঞ্জুর করা হলো।';
            }
        }
        $html .= '</p>';
        ...
        if ($sigUserData && !empty($sigUserData['signature'])) {
            $sig = $sigUserData['signature'];
            ...
            if (!empty($sigBase64)) {
                ...
                $html .= '<span class="small-text">' . banglaNumber(date_format($officeNoticeDate, "d.m.Y")) . '</span><br>';
            }
        }
        ...
        if (!empty($copyToList)) {
            ...
            foreach ($copyToList as $copy) {
                ...
                if ($copyEmpData) {
                    // Get designation
                    $stmt = $con->prepare("SELECT * FROM job_title WHERE id = ?");
                    $stmt->bind_param("i", $copy['designation_id']);
                    $stmt->execute();
                    ...
                    // Get section
                    $stmt = $con->prepare("SELECT * FROM sections WHERE id = ?");
                    $stmt->bind_param("i", $copy['section_id']);
                    $stmt->execute();
                    ...
                    // Get organization
                    $stmt = $con->prepare("SELECT * FROM organization WHERE id = ?");
                    $stmt->bind_param("i", $copy['organization_id']);
                    $stmt->execute();
        ...
        $mpdf->SetTitle('Office Order - ' . $empData['employee_name']);
        $mpdf->WriteHTML($html);
        ...
        echo json_encode([
            'success' => true,
            'pdfData' => $base64,
            'title' => 'Office Order - ' . $empData['employee_name']
        ]);
...
<head>
    ...
    <title>Office Order - ID: <?= $leaveApplicationID ?></title>
    ...
    <script>
        ...
        function downloadPDF() {
            if (pdfDataBlob) {
                ...
                a.href = url;
                a.download = `office_order_${leaveApplicationID}.pdf`;
                a.click();
