...
$menuslug = isset($_GET['menuslug']) ? $_GET['menuslug'] : 'approved-leave-deduction-data';
...
// Frontend column order: 0=sl, 1=employee_cell, 2=leave_type, 3=deduction, 4=note, 5=submit_date, 6=attachment, 7=status
$columnsMap = [
    0 => 'leave_deduction_history.dataID',
    1 => 'employee_list.employee_name',
    2 => 'leave_deduction_history.leaveID',
    3 => 'leave_deduction_history.leaveDeduction',
    4 => 'leave_deduction_history.note',
    5 => 'leave_deduction_history.createDate',
    6 => 'leave_deduction_history.dataID',
    7 => 'leave_deduction_history.isApproved',
];
$orderColumn = $columnsMap[$orderColumnIndex] ?? 'leave_deduction_history.createDate';
...
if (!empty($_SESSION['isCenterAdmin']) && !empty($_SESSION['centerAdminOrgID'])) {
    ...
    $stmt_org->execute();
    $orgID = (int)($stmt_org->get_result()->fetch_assoc()['organization_id'] ?? 0);
    $stmt_org->close();
}
...
$filterClause = '';
// Only superadmin (orgID=0) can filter by center; others are pinned to their org
if ($orgID === 0 && $centerFilter > 0) $filterClause .= " AND employee_list.organization_id = $centerFilter";
...
if ($leaveTypeFilter > 0) $filterClause .= " AND leave_deduction_history.leaveID = $leaveTypeFilter";
if ($statusFilter !== '' && in_array($statusFilter, ['0','1','2'], true)) {
    $filterClause .= " AND leave_deduction_history.isApproved = $statusFilter";
}
if ($dateFromF !== '' && $dateToF !== '' && preg_match('/^\d{4}-\d{2}-\d{2}$/', $dateFromF) && preg_match('/^\d{4}-\d{2}-\d{2}$/', $dateToF)) {
    $filterClause .= " AND leave_deduction_history.createDate BETWEEN '$dateFromF' AND '$dateToF'";
}

$baseFrom = "FROM leave_deduction_history
             INNER JOIN employee_list ON leave_deduction_history.employeeID = employee_list.id
             ...
             WHERE employee_list.organization_id = $orgID
             $filterClause";

// Total records (with filters but no per-search)
...
if (!empty($searchValue)) {
    ...
    $searchSql = " AND (
        ...
        leave_deduction_history.note LIKE '%$sv%' OR
        leave_deduction_history.createDate LIKE '%$sv%'
    )";
...
// Data query — pull audit-trail columns + actor names via subqueries
// (kept as LEFT JOIN aliases so the existing $baseFrom WHERE/COUNT queries
// don't need to change).
$selectFields = "leave_deduction_history.dataID,
                 leave_deduction_history.employeeID,
                 leave_deduction_history.leaveID,
                 leave_deduction_history.leaveDeduction,
                 leave_deduction_history.note,
                 leave_deduction_history.createDate,
                 leave_deduction_history.attachment,
                 leave_deduction_history.isApproved,
                 leave_deduction_history.approvedDate,
                 leave_deduction_history.rejectedDate,
                 leave_deduction_history.rejectionReason,
                 ...
                 (SELECT CONCAT_WS('|', COALESCE(ael.employee_name, aul.full_name, aul.user_id), aul.user_id)
                  ...
                  WHERE aul.dataID = leave_deduction_history.approvedBy LIMIT 1) AS approved_by_label,
                 (SELECT CONCAT_WS('|', COALESCE(rel.employee_name, rul.full_name, rul.user_id), rul.user_id)
                  ...
                  WHERE rul.dataID = leave_deduction_history.rejectedBy LIMIT 1) AS rejected_by_label";
...
while ($row = mysqli_fetch_assoc($result)) {
    ...
    // Deduction days
    $dedHtml = '<span class="days-pill days-pill-warning">' . banglaNumber((int)$row['leaveDeduction']) . ' দিন</span>';
