...
// Base query - Pending leaves (isSentbyAdmin = 0)
...
while ($empRow = mysqli_fetch_array($dataResult)) {
    ...
    if ($empRow['joiningType'] == 1 || $empRow['joiningType'] == 2 || $empRow['joiningType'] == 3) {
        ...
        if ($empRow['isSentbyAdmin'] == 0 && $empRow['joiningType'] != 0) {
            $actionHtml .= '<a class="action-icon icon-approve" href="../../views/leave/joining-update.php?menuslug=manage-approved-leaves&leaveApplicationID=' . intval($empRow['leaveApplicationID']) . '&isApproved=1" data-bs-toggle="tooltip" title="ফরওয়ার্ড">
