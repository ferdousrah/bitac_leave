...
while ($empRow = mysqli_fetch_array($query)) {
    ...
    if ($proceed == 1) {
        ...
        // Multi-segment breakdown — same convention as the other queue fetches:
        // >1 segment replaces the single "N দিন <type>" pill with a total-days
        // pill + per-segment chips.
        $__segQ = mysqli_query($con, "SELECT s.*, lt.leaveTitle
                                       ...
                                       WHERE s.applicationID = " . (int)$leaveAppID . "
