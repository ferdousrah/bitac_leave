...
// DB credentials — read from env vars in production, fall back to local XAMPP defaults
$host    = getenv('DB_HOST')     ?: 'localhost';
$user    = getenv('DB_USER')     ?: 'root';
$pass    = getenv('DB_PASSWORD') ?: '';
$dbName  = getenv('DB_NAME')     ?: 'bitac_leave_dev';
$dbPort  = (int)(getenv('DB_PORT') ?: 3306);

$con = mysqli_connect($host, $user, $pass, $dbName, $dbPort);
...
// Use BASE_URL from paths.php instead of hardcoded value
$baseURL = BASE_URL . "/";

$getSettingsDetailsQ = mysqli_query($con, "select * from template_settings where dataID=1");
$getSettingsDetailsQRW = mysqli_fetch_assoc($getSettingsDetailsQ);
...
// Include shared utility functions (getEmployeeLeaveInfo, calculateLeave, banglaNumber, etc.)
if (file_exists(ROOT_PATH . '/function.php')) {
    require_once ROOT_PATH . '/function.php';
}

function convertDateTotrad($date){
