...
// Get employees with approved increments
$getAllEmployeeListQ = mysqli_query($con, "SELECT DISTINCT employeeID FROM yearly_salary_increment WHERE status=1 ORDER BY employeeID ASC");

$menuslug = htmlspecialchars($_GET['menuslug'] ?? 'increment-file-permission');
?>

<!-- Page Header -->
<div class="row mb-4 align-items-center">
    <div class="col-12 col-md-7">
        <h4 class="fw-bold mb-0"><i class="ti tabler-file-certificate me-2 text-primary"></i>ইনক্রিমেন্ট ফাইলে অনুমোদন</h4>
        <div class="text-muted small mt-1 ms-1"><i class="ti tabler-info-circle me-1"></i>একজন কর্মচারীর জন্য ইনক্রিমেন্ট ফাইল জেনারেট করুন</div>
    </div>
...
<!-- Increment File Permission Form Card -->
<div class="card simple-form-card shadow-sm border-0">
    <div class="card-body">
        <form action="file-permission-print.php" method="post" id="incrementForm" onsubmit="target_popup(this)" data-turbo="false">
            <!-- Section header -->
            <div class="form-section-header">
                <span class="section-icon-tile"><i class="ti tabler-file-export"></i></span>
                <h6 class="section-title">ফাইল জেনারেশন</h6>
            </div>
            ...
            <div class="row mb-3">
                ...
                <div class="col-md-9">
                    <select class="js-example-basic-single" style="width: 100%;" name="employeeID" id="employeeID" required>
                        ...
                        <?php
                        if ($getAllEmployeeListQ && mysqli_num_rows($getAllEmployeeListQ) > 0) {
                            while($empRow = mysqli_fetch_array($getAllEmployeeListQ)) {
                                $getEmployeeDetailsQ = mysqli_query($con, "SELECT * FROM employee_list WHERE id='" . intval($empRow['employeeID']) . "'");
                                $getEmployeeDetailsQRW = mysqli_fetch_assoc($getEmployeeDetailsQ);

                                if ($getEmployeeDetailsQRW):
                        ?>
                            <option value='<?= intval($getEmployeeDetailsQRW['id']) ?>'>
                                <?= $obj->engToBn($getEmployeeDetailsQRW['employee_id']) . ' - ' . htmlspecialchars($getEmployeeDetailsQRW['employee_name']) ?>
                            </option>
                        <?php
                                endif;
                            }
                    ...
                    <small class="text-muted mt-1 d-block"><i class="ti tabler-info-circle me-1"></i>যে কর্মচারীর জন্য ফাইল জেনারেট করতে চান তাকে নির্বাচন করুন</small>
                </div>
            </div>
            ...
            <div class="row mb-3">
                ...
                <div class="col-md-9">
                    ...
                    <small class="text-muted mt-1 d-block"><i class="ti tabler-info-circle me-1"></i>যে অর্থ বছরের জন্য ফাইল জেনারেট করতে চান তা নির্বাচন করুন</small>
                </div>
            </div>
            ...
            <div class="row mb-3">
                <div class="col-md-9 offset-md-3">
                    <div class="popup-info-banner">
                        ...
                        <div><strong>নোট:</strong> ফাইলটি একটি নতুন উইন্ডোতে খুলবে। অনুগ্রহ করে পপ-আপ ব্লকার নিষ্ক্রিয় করুন।</div>
                    </div>
                </div>
            </div>
            ...
            <div class="simple-form-actions d-flex gap-2 justify-content-end">
                ...
                <button type="submit" name="submit" id="submit" class="btn btn-primary px-4">
                    <i class="ti tabler-file-export me-1"></i>জেনারেট করুন
                </button>
            </div>
        </form>
    </div>
...
$(document).ready(function() {
    ...
    $('#incrementForm').on('submit', function() {
        ...
        setTimeout(function() {
            ...
            submitBtn.html('<i class="ti tabler-file-export me-1"></i>জেনারেট করুন');
        }, 2000);
    });
});
