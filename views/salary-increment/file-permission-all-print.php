...
if ($sectionID == 0) {
    $sectionSTR = "বিটাক";
} else if (in_array($sectionID, [36, 38, 39, 40, 41])) {
...
// Get employee list based on section
if ($sectionID == 0) {
...
// Get approved date
$getSalaryIncrementDateQ = mysqli_query($con, "SELECT * FROM yearly_salary_increment WHERE incrementYear='" . $incrementYear . "' ORDER BY approvedDate DESC LIMIT 1");
$getSalaryIncrementDateQRW = mysqli_fetch_assoc($getSalaryIncrementDateQ);
?>
<!DOCTYPE html>
<html>
<head>
    <meta charset="UTF-8">
    <title>ইনক্রিমেন্ট নোটে অনুমোদন (সকল)</title>
    <style>
...
<div id="ele1" style="padding:0px; margin:0; width:100%;">
        ...
        <tr>
            ...
            <td width="50%">
                <p style="font-size: 15px;" align="right">
                    নোট অনুমোদনের তারিখ: <?php if (!empty($getSalaryIncrementDateQRW['approvedDate'])) { echo $obj->engToBn(convertDateTotrad($getSalaryIncrementDateQRW['approvedDate'])); } ?> খ্রিস্টাব্দ
                </p>
            </td>
        </tr>
    </table>
    ...
    <p style="font-size: 15px;text-align: justify;">
        &nbsp;&nbsp;&nbsp;<?php echo $sectionSTR; ?> এর নিম্নোক্ত কর্মচারীগণকে অর্থ মন্ত্রণালয়, অর্থ বিভাগের বাস্তবায়ন অনুবিভাগের ১৫-১২-২০১৫ খ্রিষ্ট: তারিখের এস.আর.ও.নং-৩৭০-আইন/২০১৫ । Service(Recognition and Conditions) Act. 1975(Act No. XXXII of 1975) এর Section-5-এ প্রদত্ত আদেশের অনুচ্ছেদ ১০ অনুসারে <?php echo convertDateTotrad($obj->engToBn($getIncrementSettingsRW['salary_increment_date'] ?? '')); ?> সাল থেকে নিম্নোক্ত ছক মোতাবেক বেতন বৃদ্ধি করা যেতে পারে। উল্লেখ্য যে, নিম্নোক্ত কর্মচারীগণ বেতন বৃদ্ধির আলোচ্য বছরে কোন অবৈতনিক ছুটি ভোগ করেননি। তাদের নামে কোন বিভাগীয় মামলা/ ইনক্রিমেন্ট এর স্থগিতাদেশ/ বেতন গ্রেড অবনমিতকরণের কোন আদেশ নেই। সদয় অনুমোদনের জন্য পেশ করা হলো।
    </p>
    ...
    <table width="100%" border="1" style="border-collapse: collapse; border: 1px solid #000;font-size: 12px;" cellpadding="8" cellspacing="0">
        <tr>
            <th width="40" style="border-bottom: 1px solid #000;border-right: 1px solid #000;">ক্রমিক</th>
            <th width="200" style="border-bottom: 1px solid #000;border-right: 1px solid #000;">নাম ও পদবী</th>
        ...
        <tr>
            ...
            <td style="border-right: 1px solid #000;">
                <span style="padding: 0px;">
                    <?php echo htmlspecialchars($empRow['employee_name']); ?><br>
                    <?php echo htmlspecialchars($getDesignationDetailsQRW['job_title_name'] ?? ''); ?>
                </span>
            </td>
            ...
            <td style="text-align: center;border-right: 1px solid #000;">
                <?php echo $obj->engToBn($getPresentSGradeQRW['minimum_salary'] ?? 0) . '-' . $obj->engToBn($getPresentSGradeQRW['maximum_salary'] ?? 0); ?>
            </td>
            <td style="text-align: center;border-right: 1px solid #000;"><?php echo $obj->engToBn(number_format($empRow['presentSalary'])); ?></td>
