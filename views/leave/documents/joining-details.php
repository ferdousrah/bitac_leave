<?php
/**
 * Leave Joining Application Viewer - In-Memory Version
 * Shows employee's application to rejoin work after leave
 * 
 * Usage: leave_joining_viewer.php?leaveApplicationID=5516
 */
...
if ($leaveApplicationID <= 0) {
    ...
    <body>
        <div class="error-box">
            ...
            <div class="info">
                <strong>Usage:</strong>
                <div class="example">leave_joining_viewer.php?leaveApplicationID=<strong>5516</strong></div>
            </div>
        </div>
    </body>
    </html>
...
function generatePDFData($leaveApplicationID) {
    try {
        ...
        // Date at top
        $html .= '<p class="text-right">তারিখ: ' . banglaNumber(date_format($submitDate, "d/m/Y")) . '</p>';
        ...
        // Subject
        $html .= '<p>বিষয়ঃ ছুটি ভোগ শেষে কর্মস্থলে যোগদান প্রসঙ্গে।</p>';
        $html .= '<p>&nbsp;</p>';
        ...
        $html .= ' অনুযায়ী ';
        $html .= banglaNumber(date_format($dateF, "d/m/Y")) . ' থেকে ';
        $html .= banglaNumber(date_format($dateT, "d/m/Y")) . ' তারিখ পর্যন্ত, ';
        $html .= banglaNumber($dateDiff) . ' দিনের ছুটি ভোগ শেষে কর্মস্থলে যোগদান করতে ইচ্ছুক।</p>';
        
        // Request paragraph
        $html .= '<p>&nbsp;&nbsp;&nbsp;অতএব, মহোদয়ের নিকট বিনীত নিবেদন আমার যোগদান পত্রখানা গ্রহণকরতঃ আমাকে কর্মস্থলে যোগদানের অনুমতি প্রদান করে বাধিত করবেন।</p>';
        ...
        $html .= '<tr>';
        $html .= '<td width="60%">&nbsp;</td>';
        $html .= '<td width="40%" class="signature-section">';
        ...
        // Section in-charge note
        if ($secInchargeNote && !empty($secInchargeNote['note'])) {
            $html .= '<p>' . htmlspecialchars($secInchargeNote['note']) . '</p>';
        }
        ...
        $mpdf->SetTitle('Leave Joining Application - ' . $empData['employee_name']);
        $mpdf->WriteHTML($html);
        ...
        echo json_encode([
            'success' => true,
            'pdfData' => $base64,
            'title' => 'Leave Joining Application - ' . $empData['employee_name']
        ]);
...
<head>
    ...
    <title>Leave Joining Application - ID: <?= $leaveApplicationID ?></title>
...
<body>
    <div class="header">
        <h1>🏢 Leave Joining Application - ID: <?= $leaveApplicationID ?></h1>
    </div>
    ...
    <script>
        ...
        function downloadPDF() {
            if (pdfDataBlob) {
                ...
                a.href = url;
                a.download = `leave_joining_${leaveApplicationID}.pdf`;
                a.click();
