<?php
/**
 * Extended Leave Joining Report Viewer - In-Memory Version
 * Shows employee request for extended leave and rejoining
 * 
 * Usage: extended_leave_joining_viewer.php?leaveApplicationID=5516
 */
...
if ($leaveApplicationID <= 0) {
    ...
    <body>
        <div class="error-box">
            ...
            <div class="info">
                <strong>Usage:</strong>
                <div class="example">extended_leave_joining_viewer.php?leaveApplicationID=<strong>5516</strong></div>
            </div>
        </div>
    </body>
    </html>
...
function generatePDFData($leaveApplicationID) {
    try {
        ...
        $stmt->bind_param("s", $joiningData['approvedLeaveType']);
        $stmt->execute();
        ...
        // Subject
        $html .= '<p>বিষয়ঃ বর্ধিত ছুটি মঞ্জুতকরতঃ কর্মস্থলে যোগদানের অনুমতি প্রদান প্রসঙ্গে।</p><br>';
        
        // Salutation
        $html .= '<p style="padding-top: 0px; margin-top: 0px;">মহোদয়,</p>';
        ...
        $html .= ' দিনের ছুটি শেষে ' . htmlspecialchars($joiningData['reason']);
        $html .= ' সঠিক সময়ে অফিসে আসতে পারিনি যা আমি কর্তৃপক্ষকে পূর্বে অবগত করেছি।</p>';
        ...
        $mpdf->SetTitle('Extended Leave Joining - ' . $empData['employee_name']);
        $mpdf->WriteHTML($html);
        ...
        echo json_encode([
            'success' => true,
            'pdfData' => $base64,
            'title' => 'Extended Leave Joining - ' . $empData['employee_name']
        ]);
...
<head>
    ...
    <title>Extended Leave Joining - ID: <?= $leaveApplicationID ?></title>
    ...
    <script>
        ...
        function downloadPDF() {
            if (pdfDataBlob) {
                ...
                a.href = url;
                a.download = `extended_leave_joining_${leaveApplicationID}.pdf`;
                a.click();
