...
// NO Super Admin bypass on purpose: the approve endpoint only accepts the
// assigned signatory, so showing rows to non-signatories would produce
// clickable buttons that fail. Keep count == fetch == actionable.
$viewerOrgID  = (int)($getUserInfoQRW['organization_id'] ?? 0);
$viewerEmpID  = (int)($getUserInfoQRW['employee_id']     ?? 0);
// Used further down to render the "center" filter dropdown (super admins see
// the org selector; center admins are pinned).
...
if ($isOrgSignatory) {
    $scopeSql = " AND (ldh.override_signatory_id = $viewerEmpID
                       OR (ldh.override_signatory_id IS NULL AND el.organization_id = $viewerOrgID))";
} else {
    $scopeSql = " AND ldh.override_signatory_id = $viewerEmpID";
}
$pendingCount = (int)(mysqli_fetch_assoc(mysqli_query($con,
    "SELECT COUNT(DISTINCT COALESCE(ldh.batch_id, CONCAT('_solo_', ldh.dataID))) AS c
     ...
     WHERE ldh.isApproved = 0 $scopeSql"))['c'] ?? 0);
...
<div class="row mb-4 align-items-center">
    <div class="col-12 col-md-6">
        <h4 class="fw-bold mb-0"><i class="ti tabler-edit-circle me-2 text-primary"></i>ছুটি কর্তনের অনুমোদন</h4>
    </div>
...
<div class="row stats-strip mb-3 g-2">
    <div class="col-12 col-md-6 col-lg-4">
        <div class="stat-card stat-pending"
             ...
             title="অনুমোদনের জন্য অপেক্ষমান ছুটি কর্তনের সংখ্যা">
...
<div class="card leave-apps-card shadow-sm border-0">
    <div class="card-body p-3">
        <!-- Filter Panel -->
        <div class="filter-panel mb-3 is-collapsed" data-scope="reg">
            <div class="filter-panel-header">
                <button type="button" class="filter-panel-toggle" data-scope="reg" aria-expanded="false" aria-controls="filterBody-reg">
                    <i class="ti tabler-filter me-1"></i>
                    <span class="filter-panel-title">ফিল্টার</span>
                    <span class="filter-active-count" data-scope="reg"></span>
                    <i class="ti tabler-chevron-down filter-chevron ms-2"></i>
                </button>
                <div class="filter-panel-actions">
                    <button type="button" class="btn btn-sm btn-icon btn-label-primary table-refresh" data-scope="reg" title="টেবিল রিফ্রেশ" data-bs-toggle="tooltip">
                        <i class="ti tabler-refresh"></i>
                    </button>
                    <button type="button" class="btn btn-sm btn-label-secondary filter-reset" data-scope="reg">
                        <i class="ti tabler-x me-1"></i>রিসেট
                    </button>
                </div>
            </div>
            <div class="filter-panel-body" id="filterBody-reg">
                <div class="row g-2">
                    <?php if ($isSuperAdmin): ?>
                    <div class="col-12 col-md-6 col-lg-3">
                        <label class="filter-label"><i class="ti tabler-map-pin"></i>কেন্দ্র</label>
                        <select id="regOrgFilter" class="form-select form-select-sm filter-input" data-scope="reg">
                    ...
                    <div class="col-12 col-md-6 col-lg-3">
                        ...
                        <select id="regSectionFilter" class="form-select form-select-sm filter-input" data-scope="reg">
                    ...
                    <div class="col-12 col-md-6 col-lg-3">
                        ...
                        <select id="regEmployeeFilter" class="form-select form-select-sm filter-input" data-scope="reg">
                            <option value="">সকল</option>
                        </select>
                    </div>
                    <div class="col-12 col-md-6 col-lg-3">
                        <label class="filter-label"><i class="ti tabler-clipboard-list"></i>ছুটির ধরণ</label>
                        <select id="regLeaveTypeFilter" class="form-select form-select-sm filter-input" data-scope="reg">
...
function buildRegPayload() {
    return {
        centerFilter:    $('#regOrgFilter').val()         || '',
        sectionFilter:   $('#regSectionFilter').val()     || '',
        employeeFilter:  $('#regEmployeeFilter').val()    || '',
        leaveTypeFilter: $('#regLeaveTypeFilter').val()   || ''
    };
}

function updateRegActiveCount() {
    var p = buildRegPayload();
    var n = 0;
    if (p.centerFilter)    n++;
    if (p.sectionFilter)   n++;
    if (p.employeeFilter)  n++;
    if (p.leaveTypeFilter) n++;
    var $b = $('.filter-active-count[data-scope="reg"]');
    if (n > 0) $b.text(n).addClass('has-active');
    else       $b.text('').removeClass('has-active');
    ['#regOrgFilter', '#regSectionFilter', '#regEmployeeFilter', '#regLeaveTypeFilter'].forEach(function(sel){
        var $el = $(sel);
        if (!$el.length) return;
...
function initRegEmployeeSelect() {
    $('#regEmployeeFilter').select2({
        ...
        ajax: {
            ...
            data: function(params) {
                return { q: params.term || '', org: $('#regOrgFilter').val() || 0, page: params.page || 1 };
            },
...
$(document).ready(function() {
    $('#regOrgFilter, #regSectionFilter, #regLeaveTypeFilter').select2({ width: '100%', allowClear: true, placeholder: '— সকল —' });
    initRegEmployeeSelect();

    regLeaveDT = $('#leaveDeductionTable').DataTable({
        processing: true,
        serverSide: true,
        responsive: false,
        autoWidth: false,
        ajax: {
            url: "../../api/employees/fetch-regular-leave-approval.php",
            type: "POST",
            data: function(d) { Object.assign(d, buildRegPayload()); }
        },
        columns: [
            { data: "serial",          orderable: false },
            { data: "employee_info",   orderable: true },
            { data: "leave_type",      orderable: true },
            { data: "leave_deduction", orderable: true },
        ...
        createdRow: function(row) {
            var labels = ['ক্রমিক', 'কর্মচারী', 'ছুটির ধরণ', 'কর্তন (দিন)', 'মন্তব্য', 'অফিস আদেশ', 'কার্যাবলী'];
        ...
        language: {
            ...
            emptyTable: '<div class="empty-state-rich"><i class="ti tabler-clipboard-off"></i><div class="empty-title">কোন কর্তন নেই</div><div class="empty-subtitle">এই মুহূর্তে কোনো ছুটি কর্তন অনুমোদনের অপেক্ষায় নেই</div></div>',
    ...
    $(document).on('change', '.filter-input', function() {
        updateRegActiveCount();
        regLeaveDT.ajax.reload();
    });
    $(document).on('click', '.filter-reset', function(e){
        e.stopPropagation();
        $('#regOrgFilter, #regSectionFilter, #regEmployeeFilter, #regLeaveTypeFilter').val(null).trigger('change.select2');
        updateRegActiveCount();
        regLeaveDT.ajax.reload();
    });
...
function approveRegularLeaveInfo(batchKey, isApproved) {
    ...
    Swal.fire(swalConfig).then(function (result) {
        ...
        $.ajax({
            type: 'POST',
            url: '../../api/employees/approve-regular-leave-info-action.php',
            data: payload,
            dataType: 'json',
            success: function (response) {
                if (response.status === 'success') {
                    Swal.fire({
                        ...
                        buttonsStyling: false
                    }).then(function () { regLeaveDT.ajax.reload(); });
                } else {
                    Swal.fire({
