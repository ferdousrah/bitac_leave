...
$menuslug = htmlspecialchars($_GET['menuslug'] ?? 'leave-deduction-on-office-order');
?>

<!-- Page Header -->
<div class="row mb-4 align-items-center">
    <div class="col-12 col-md-7">
        <h4 class="fw-bold mb-0"><i class="ti tabler-calendar-minus me-2 text-primary"></i>অফিস আদেশে ছুটি কর্তন</h4>
        <div class="text-muted small mt-1 ms-1"><i class="ti tabler-info-circle me-1"></i>অফিস আদেশের ভিত্তিতে কর্মচারীর ছুটি কর্তন এন্ট্রি করুন</div>
    </div>
...
.deduct-section-header[data-color="amber"]  { --sec-bg: #fff3e1; --sec-accent: #b8651a; }
        ...
        <form class="form-horizontal" name="form" id="form" enctype="multipart/form-data">
            ...
            <div class="deduct-section-header" data-color="indigo">
                ...
                <div class="section-text">
                    ...
                    <span class="section-sub">যে কর্মচারীর ছুটি কর্তন করা হবে তাকে নির্বাচন করুন</span>
                </div>
                    ...
                    <select class="select2" style="width: 100%;" name="employeeID" id="employeeID" data-allow-clear="true" required>
                        ...
                        <?php while($empRow = mysqli_fetch_array($getEmployeeListQ)): ?>
                            <option value='<?= $empRow['id'] ?>'>
                                <?= Bengali_DTN($empRow['employee_id']) . ' - ' . htmlspecialchars($empRow['employee_name']) ?>
                            </option>
                        <?php endwhile; ?>
                    </select>
                </div>
            ...
            <div class="deduct-section-header" data-color="amber">
                <div class="section-num">২</div>
                <div class="section-text">
                    <h6 class="section-title">ছুটি কর্তন তথ্য</h6>
                    <span class="section-sub">ছুটির ধরন, কর্তনের দিন, মন্তব্য ও অফিস আদেশ সংযুক্তি</span>
                </div>
                <span class="section-icon"><i class="ti tabler-calendar-minus"></i></span>
            </div>
            ...
            <div class="row mb-3">
                ...
                <div class="col-md-9">
                    <div class="table-responsive">
                        <table class="table table-bordered align-middle mb-2" id="leaveRowsTable">
                            <thead class="table-light">
                                <tr>
                                    <th style="width:32%;">ডিডাক্ট ফ্রম</th>
                    ...
                    <small class="text-muted d-block mt-1"><i class="ti tabler-info-circle me-1"></i>এই অফিস আদেশে যতগুলো ছুটি কর্তন করতে চান, ততগুলো সারি যোগ করুন</small>
                </div>
            </div>
            ...
            <div class="row mb-3">
                ...
                <div class="col-md-9">
                    <input type="file" class="form-control" id="officeAdesh" name="officeAdesh" accept=".pdf,.jpg,.jpeg,.png" required>
                    <small class="text-muted mt-1 d-block"><i class="ti tabler-info-circle me-1"></i>PDF, JPG বা PNG ফরম্যাট সমর্থিত। সব কর্তনের জন্য একই আদেশ প্রযোজ্য হবে।</small>
                </div>
            </div>
...
(function bootDeductionPage() {
    if (typeof jQuery === 'undefined' || !jQuery.fn || typeof Swal === 'undefined') {
        return setTimeout(bootDeductionPage, 30);
    }
    ...
    var LEAVE_TYPES = [
        ...
        { v: '5',        t: 'ঐচ্ছিক ছুটি' },
        { v: '6',        t: 'কর্তনহীন ছুটি' },
        { v: 'shranti',  t: 'শ্রান্তি বিনোদন (গড় বেতন থেকে)' }
    ];

    // Default next maturity date = today + 3 years (BSR: 3-year recreation cycle)
    ...
    function newLeaveRow() {
        return '<tr>'
            + '<td>'
            +   '<select class="form-select form-select-sm rowTypeSelect" name="leaveType[]" required>' + typeOptions() + '</select>'
            +   '<div class="shranti-extra mt-2 d-none">'
            +     '<label class="form-label small text-muted mb-1"><i class="ti tabler-calendar-event me-1"></i>পরবর্তী শ্রান্তি ম্যাচিউর তারিখ</label>'
            +     '<input type="text" class="form-control form-control-sm flatpickr-shranti" name="maturity_date[]" placeholder="YYYY-MM-DD" value="">'
            +     '<small class="text-muted">Default: আজ + ৩ বছর</small>'
            +   '</div>'
            + '</td>'
            + '<td><input type="number" step="0.5" min="0" class="form-control form-control-sm text-center" name="leaveDeduct[]" placeholder="দিন" required></td>'
    ...
    function init() {
        ...
        $(document).off('change.empSigDed', '#employeeID').on('change.empSigDed', '#employeeID', function() {
        ...
        $(document).off('click.addLeaveD', '#addLeaveRowBtn').on('click.addLeaveD', '#addLeaveRowBtn', function() {
            $('#leaveRowsBody').append(newLeaveRow());
        });

        $(document).off('click.rmLeaveD', '.removeLeaveRow').on('click.rmLeaveD', '.removeLeaveRow', function() {
        ...
        // Show/hide shranti maturity input when row's leave type changes
        $(document).off('change.shrantiToggle', '.rowTypeSelect').on('change.shrantiToggle', '.rowTypeSelect', function() {
            var $row   = $(this).closest('tr');
            var $extra = $row.find('.shranti-extra');
            var $matInp= $row.find('.flatpickr-shranti');
            if ($(this).val() === 'shranti') {
                $extra.removeClass('d-none');
                $matInp.attr('required', 'required');
                if (!$matInp.val()) $matInp.val(defaultMaturityDate());
                if (typeof flatpickr !== 'undefined' && !$matInp[0]._flatpickr) {
                    flatpickr($matInp[0], { dateFormat: 'Y-m-d', allowInput: true });
                }
            } else {
                $extra.addClass('d-none');
                $matInp.removeAttr('required').val('');
            }
        });

        function doSubmit(formEl) {
            var $btn = $('.submitBtn');
            $btn.prop('disabled', true).html('<span class="spinner-border spinner-border-sm me-2"></span>প্রক্রিয়াকরণ হচ্ছে...');
            $('#form').css("opacity", ".5");

            $.ajax({
                url: '../../api/leave-deduction/save.php',
                type: 'POST',
                dataType: 'json',
                ...
                error: function() {
                    Swal.fire({ title: 'ত্রুটি', text: 'ছুটি কর্তন সংরক্ষণ করতে ব্যর্থ হয়েছে!', icon: 'error', confirmButtonColor: '#ff3e1d', customClass: { confirmButton: 'btn btn-danger' }, buttonsStyling: false });
                },
                complete: function() {
                    $('#form').css("opacity", "");
        ...
        $('#submit').off('click.submitDed').on('click.submitDed', function(e) {
            e.preventDefault();
            ...
            // Any shranti rows? If yes, check the employee's last recreation eligibility.
            var hasShranti = false;
            $('.rowTypeSelect').each(function() { if ($(this).val() === 'shranti') hasShranti = true; });
            var empId = $('#employeeID').val();

            if (hasShranti && empId) {
                $.get('../../api/leave-deduction/check-shranti-eligibility.php', { employee_id: empId }, function(resp) {
                    if (resp && resp.status === 1 && resp.has_history && !resp.eligible) {
                        Swal.fire({
                            title: 'সতর্কতা: এই কর্মচারী এখনো ম্যাচিউর নন',
                            html: '<div class="text-start">'
                                + '<div class="mb-2"><small class="text-muted">শেষ শ্রান্তি কর্তন:</small><br><b>' + resp.last_deducted_on + '</b></div>'
                                + '<div class="mb-2"><small class="text-muted">পরবর্তী eligible তারিখ:</small><br><b>' + resp.next_maturity_date + '</b> <span class="text-warning">(' + resp.days_until_maturity + ' দিন বাকি)</span></div>'
                                + '<hr><div>তবুও শ্রান্তি কর্তন করতে চান?</div>'
                                + '</div>',
                            icon: 'warning',
                            showCancelButton: true,
                            confirmButtonColor: '#d97706',
                            cancelButtonColor: '#8592a3',
                            confirmButtonText: 'হ্যাঁ, প্রক্রিয়া করুন',
                            cancelButtonText: 'বাতিল',
                            customClass: { confirmButton: 'btn btn-warning me-2', cancelButton: 'btn btn-label-secondary' },
                            buttonsStyling: false
                        }).then(function(r) {
                            if (r.isConfirmed) doSubmit(formEl);
                        });
                    } else {
                        doSubmit(formEl);
                    }
                }, 'json').fail(function() {
                    // On check failure, still allow submit — don't block due to network error
                    doSubmit(formEl);
                });
            } else {
                doSubmit(formEl);
